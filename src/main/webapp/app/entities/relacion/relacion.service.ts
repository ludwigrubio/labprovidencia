...
export class RelacionService {
  ...
  create(relacion: IRelacion): Observable<EntityResponseType> {
    return this.http.post<IRelacion>(this.resourceUrl, relacion, { observe: 'response' });
  }

  update(relacion: IRelacion): Observable<EntityResponseType> {
    return this.http.put<IRelacion>(this.resourceUrl, relacion, { observe: 'response' });
  }

  find(id: number): Observable<EntityResponseType> {
    return this.http.get<IRelacion>(`${this.resourceUrl}/${id}`, { observe: 'response' });
  }

  query(req?: any): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http.get<IRelacion[]>(this.resourceUrl, { params: options, observe: 'response' });
  }
