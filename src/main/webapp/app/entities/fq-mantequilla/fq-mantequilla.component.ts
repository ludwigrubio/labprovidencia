...
@Component({
  selector: 'jhi-fq-mantequilla',
  templateUrl: './fq-mantequilla.component.html',
})
export class FQMantequillaComponent implements OnInit, OnDestroy {
  fQMantequillas?: IFQMantequilla[];
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.fQMantequillaService
      ...
      .subscribe(
        (res: HttpResponse<IFQMantequilla[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  ngOnInit(): void {
    ...
    this.registerChangeInFQMantequillas();
  }
  ...
  trackId(index: number, item: IFQMantequilla): number {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInFQMantequillas(): void {
    this.eventSubscriber = this.eventManager.subscribe('fQMantequillaListModification', () => this.loadPage());
  }

  delete(fQMantequilla: IFQMantequilla): void {
    const modalRef = this.modalService.open(FQMantequillaDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.fQMantequilla = fQMantequilla;
  }
  ...
  protected onSuccess(data: IFQMantequilla[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/fq-mantequilla'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
