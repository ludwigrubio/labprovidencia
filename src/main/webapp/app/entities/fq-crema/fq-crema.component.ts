...
@Component({
  selector: 'jhi-fq-crema',
  templateUrl: './fq-crema.component.html',
})
export class FQCremaComponent implements OnInit, OnDestroy {
  fQCremas?: IFQCrema[];
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.fQCremaService
      ...
      .subscribe(
        (res: HttpResponse<IFQCrema[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: IFQCrema): number {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInFQCremas(): void {
    this.eventSubscriber = this.eventManager.subscribe('fQCremaListModification', () => this.loadPage());
  }

  delete(fQCrema: IFQCrema): void {
    const modalRef = this.modalService.open(FQCremaDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.fQCrema = fQCrema;
  }
  ...
  protected onSuccess(data: IFQCrema[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/fq-crema'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
