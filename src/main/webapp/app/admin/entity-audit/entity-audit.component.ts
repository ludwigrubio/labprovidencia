...
export class EntityAuditComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.service.getAllAudited().subscribe(entities => {
      entities.forEach(entity => this.entitiesSimple.push((entity || '').split('.').pop()!));
  ...
  loadChanges(): void {
    ...
    this.loading = true;
    this.service.findByEntity(this.entities.filter(s => s.includes(this.selectedEntity || ''))[0], this.selectedLimit).subscribe(
      res => {
        const data = res.body || [];
